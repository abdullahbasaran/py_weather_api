...
for col in ['ILCEADI']:
    excel_data_df[col] = excel_data_df[col].apply(unidecode)
...
while dt < end:
    ...
    while i < 973:
        ...
        ilce = excel_data_df.loc[i][5]
        kod_ilce = excel_data_df.loc[i][3]

        if ilce == ' Merkez':
            ilce = excel_data_df.loc[i][4]
        i += 1
        ...
        # istenilen degerlerin cekilmesi
...
schedule.every(1).hour.do(WriteToExcel) #excel schedule ile yazma
while True:
